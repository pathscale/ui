...
# Include the library source and compiled package: checking only the fixture
# let a component edit or library rebuild pass against an older harness bundle.
# The generators execute outside the page; their generated entries are inputs.
newest_source="$(find "$ROOT/src" "$ROOT/dist" "$HERE" \
  \( -path "$HERE/dist" -o -name node_modules \) -prune -o \
  -type f \( -name '*.tsx' -o -name '*.ts' -o -name '*.js' -o -name '*.css' \) \
  -newer "$reference" -print 2>/dev/null | grep -vE '/generate-[^/]*\.ts$' | head -1)"
...
#
# `full` is the release contract on both local machines and Linux CI. Build
# the host with system-fonts and install fonts to verify actual painted text.
# The reduced `headless` profile is diagnostic coverage for fontless embedders;
# its weaker layout checks cannot establish release readiness. Both profiles
# run without a window. See `PROFILES` in generate-checks.ts for their differences.
#
