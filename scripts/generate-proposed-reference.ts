...
L.push("");
L.push("```");
L.push("internet ──> server ──> peer ──> peer mic");
L.push("        │            └─────────> peer camera");
L.push("        ├──> your mic");
L.push("        └──> your camera");
L.push("```");
L.push("");
L.push("**\"Show the worst one\" is the wrong answer**, and it is wrong in the way that costs support time. If the internet is down then six of the seven are also down, and reporting \"their camera is off\" is true, useless, and sends the user to fix the wrong thing.");
L.push("");
L.push("These are not peers, they are a dependency graph. The useful report is the **root cause**: the deepest failure whose own dependencies are healthy. Everything downstream is a symptom.");
L.push("");
L.push("```ts");
L.push("type Health = \"ok\" | \"degraded\" | \"down\" | \"unknown\";");
L.push("");
L.push("interface StatusItem {");
L.push("  id: string;");
L.push("  health: Health;");
L.push("  scope?: \"local\" | \"remote\";   // only local is actionable by this user");
L.push("  dependsOn?: string[];          // internet before server, server before media");
L.push("  label?: JSX.Element;");
L.push("  detail?: JSX.Element;");
L.push("  onRetry?: () => void;");
L.push("}");
L.push("");
L.push("summarizeStatus(items): {");
L.push("  health: Health;        // worst present — never looks better than reality");
L.push("  cause?: StatusItem;    // the one to actually report");
L.push("  failing: StatusItem[]; // the full breakdown, worst first");
L.push("  symptoms: StatusItem[];// failures explained by something upstream");
L.push("}");
L.push("```");
L.push("");
L.push("Measured against the seven-condition graph above:");
L.push("");
L.push("| Scenario | `health` | reported cause | symptoms suppressed |");
L.push("| --- | --- | --- | ---: |");
L.push("| all good | `ok` | — | 0 |");
L.push("| internet down, everything cascades | `down` | **Internet** | 6 |");
L.push("| server down | `down` | **Server** | 3 |");
L.push("| your camera off | `degraded` | Your camera | 0 |");
L.push("| their camera off | `degraded` | Their camera | 0 |");
L.push("| your mic down *and* their camera off | `down` | **Your mic** | 0 |");
L.push("| peer left | `down` | **Them** | 2 |");
L.push("| their mic unknown | `unknown` | Their mic | 0 |");
L.push("");
...
L.push("");
L.push("`degraded` outranks `unknown`: a known impairment beats a shrug. Cycles and dangling `dependsOn` ids are tolerated, so a partial set still produces a sane answer.");
L.push("");
L.push("The breakdown is never thrown away — `failing` and `symptoms` are what the expandable diagnostics panel renders, which is the part nofilter.io already built by hand.");
L.push("");
L.push("## Validation");
L.push("");
