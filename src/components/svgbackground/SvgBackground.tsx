...
export default function SvgBackground(props: SvgBackgroundProps) {
  ...
  const generateShapes = () => {
    ...
    const maxShapes = Math.min(
      200,
      Math.max(10, Math.ceil(15 + Math.sqrt(normalizedDensity) * 8)),
    );
    ...
    const coveredGrid = Array(gridSize)
      .fill(0)
    ...
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        ...
        const skipChance = Math.max(
          0,
          (0.2 - normalizedDensity / 100) * leftSideMultiplier,
        );
        ...
        const randomOffset =
          ...
          0.4 *
          (1 - Math.min(1, normalizedDensity / 50)) *
          randomOffsetMultiplier;
        const cx =
          i * cellWidth +
          cellWidth / 2 +
          (random(i * 200 + j) * 2 - 1) * randomOffset;
        const cy =
          j * cellHeight +
          cellHeight / 2 +
          (random(i * 300 + j) * 2 - 1) * randomOffset;

        const sizeRatioBase = Math.max(
          0.6,
          1.2 - Math.log10(normalizedDensity + 1) * 0.15,
        );
        ...
        const sizeVariation = isLeftSide
          ? sizeVariationBase * 1.5
          : sizeVariationBase;
        const radius = Math.max(
          minRadius,
          baseRadius * (0.8 + random(i * 400 + j) * sizeVariation),
        );

        const colorIndexBase = Math.floor(random(i * 500 + j) * colors.length);
        const colorIndex = isLeftSide
          ? Math.floor(
              (colorIndexBase + random(i * 600 + j) * colors.length) / 2,
            )
          : colorIndexBase;
        const color = colors[colorIndex];

        const opacity =
          isLeftSide && random(i * 700 + j) > 0.6
    ...
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        if (!coveredGrid[i][j]) {
          ...
          const radius = Math.max(
            minRadius,
            isLeftSide ? radiusBase * 0.8 : radiusBase,
          );
    ...
    const edgeShapes = [
      ...
      <circle
        ...
        cy="0"
        r="120"
        fill={getIntermediateColor(0.25)}
      />,
      <circle
        cx="0"
        cy="400"
        r="100"
        fill={getIntermediateColor(0.75)}
        opacity="0.7"
      />,
      <circle
        cx="400"
      ...
      <circle
        ...
        cy="200"
        r="70"
        fill={getIntermediateColor(0.65)}
        opacity="0.6"
      />,
      <circle
        cx="200"
        cy="0"
        r="100"
        fill={getIntermediateColor(0.15)}
      />,
      <circle
        cx="400"
        cy="200"
        r="100"
        fill={getIntermediateColor(0.4)}
      />,
      <circle
        cx="200"
        cy="400"
        r="100"
        fill={getIntermediateColor(0.85)}
      />,
    ];
    ...
    const accentCount = Math.max(
      0,
      maxShapes - baseShapes.length - edgeShapes.length,
    );
